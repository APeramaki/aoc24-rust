...
fn solve_part1(input: &str)-> u32 {
    ...
    let mut sum: usize = 0;
    for row in rows {
        let page_nums: Vec<_>= row.split(',').map(|num| num.trim().parse::<usize>().unwrap()).collect();
        let order_right = !page_nums
            .iter()
            .enumerate()
            .any(|(index, num)|{
                let binding = HashSet::new();
                let page_ruleset = ordering_rules.get(&num).unwrap_or(&binding);
                page_nums[..index].iter().any(|previous| page_ruleset.contains(previous))
            });
        match order_right {
